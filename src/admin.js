...
async function loadAdminUsers() {
  ...
  try {
    ...
    document.getElementById('adminUserCount').textContent = t('admin.userCount', {
      count: data.length
    });
    tbody.innerHTML = data
      .map(u => {
        const delBtn = u.is_admin
          ? ''
          : `<button class="btn" style="font-size:11px;padding:3px 8px;color:#E53;border-color:#E53" data-action="delete-user" data-email="${esc(u.email)}">${t('common.delete')}</button>`;
        const dateStr = u.added_at
          ? new Date(u.added_at).toLocaleDateString('en-US', { timeZone: 'Asia/Taipei' })
          : '—';
        return `<tr>
        <td>${esc(u.name) || '—'}</td>
