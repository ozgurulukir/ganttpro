...
export function allGroupMembersScheduled(tasks, groupId, scheduled) {
	return tasks
		...
		.every((child) => {
			if (child.type === "group")
				return allGroupMembersScheduled(tasks, child.id, scheduled);
...
export function scheduleTasks(tasks, projStart) {
	...
	const candidates = tasks.filter(
		(t) => t.type === "task" || t.type === "milestone",
	);
	...
	while (progress && iter++ < MAX) {
		...
		candidates.forEach((task) => {
			...
			const unresolvedFs = deps.filter((d) => {
				if (d.type === "group")
					return !allGroupMembersScheduled(tasks, d.id, scheduled);
			...
			if (task.type === "task") {
				...
				if (latestFfEnd) {
					const ffStart = subtractWorkingDays(
						latestFfEnd,
						(task.wday || 1) - 1,
					);
				...
				if (latestSfStart) {
					const sfStart = subtractWorkingDays(
						latestSfStart,
						(task.wday || 1) - 1,
					);
